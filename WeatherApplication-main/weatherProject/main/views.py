...
def humidity_chart(request):
    ...
    for i in reversed(data_fetched):
        data_arr.append(i.city_name)
        humid_arr.append(i.humidity)

        index_1 + 1
        if index_1 > 5:
            break

    chart_array1 = [['city_name', 'humidity'], [data_arr[0], humid_arr[0]], [data_arr[1], humid_arr[1]],
                   [data_arr[2], humid_arr[2]], [data_arr[3], humid_arr[3]], [data_arr[4], humid_arr[4]], [data_arr[5],
                                                                                                        humid_arr[5]]]

    # iterate and push to a table in html
    return render(request, "main/chart1.html",
                  {'chart_array1': json.dumps(chart_array1)})
